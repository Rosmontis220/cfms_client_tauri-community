<script lang="ts">
  ...
  import type { DeclarativeBlock, DeclarativePage } from '$lib/api/extensions';
  import type { DeclarativePageHost } from '$lib/declarative-page';
  import { notificationStore } from '$lib/stores.svelte';
  import ProgressRing from '$lib/components/ProgressRing.svelte';

  /**
   * Renders a declarative page.
   *
   * The page vocabulary is shared by the official extension interface and the
   * community plugin interface, so the renderer is shared too: it asks its
   * `host` to fetch the document and to run workflows, and knows nothing about
   * which system it is displaying.
   */
  let { host, pageId }: { host: DeclarativePageHost; pageId: string } = $props();
  ...
  async function load() {
    ...
    error = null;
    try {
      page = await host.loadPage(pageId);
      if (page.schema_version !== 1 || !Array.isArray(page.blocks)) {
        throw new Error(`Unsupported ${host.kindLabel} page schema`);
      }
      const values: Record<string, Record<string, unknown>> = {};
      for (const block of page.blocks) {
        if (block.type !== 'form') continue;
        values[block.id] = Object.fromEntries(
  ...
  async function runAction(action: Extract<DeclarativeBlock, { type: 'actions' }>['actions'][number]) {
    ...
    try {
      await host.runWorkflow(action.workflow, formValues);
    } catch (cause) {
...
{:else if error}
  <div class="extension-state extension-error" role="alert">
    <strong>Unable to open {host.kindLabel}</strong><p>{error}</p>
