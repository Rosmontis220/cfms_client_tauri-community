...
import type { DeclarativeWorkflow } from '$lib/api/extensions';
import { notificationStore } from '$lib/stores.svelte';

/**
 * Declarative workflow engine, shared by the official extension interface and
 * the community plugin interface.
 *
 * The two interfaces differ only in *who* they call and *which* capabilities
 * need the user's consent. Everything else — the node vocabulary, expression
 * evaluation, value resolution, step and wall-clock limits, cancellation — is
 * identical, so it lives here once. Two copies would drift, and a fix to one
 * would silently not reach the other.
 *
 * The engine never decides whether a capability is permitted; it asks the host
 * adapter, which is bound to whichever interface is running. Authorization
 * itself is enforced in the backend on every call.
 */
...
/** Binds the engine to one interface. */
export interface DeclarativeWorkflowHost {
  /**
   * Noun used in user-facing messages, e.g. `extension` or `community plugin`.
   * Substituted into the engine's error text so a failure names the right
   * system.
   */
  kindLabel: string;
  /** Invoke a host capability. `userConfirmed` is set only once the user agreed. */
  executeHostCall(
    capability: string,
    args: unknown,
  ...
  /**
   * Whether this capability must be confirmed by the user before it may run.
   * The official interface requires consent for downloads; the community
   * interface additionally requires it for opening a file.
   */
  requiresConfirmation(capability: string): boolean;
  /** Message shown in the consent prompt for a side-effecting capability. */
  confirmationPrompt(capability: string, args: Record<string, unknown>): string;
}

export async function runDeclarativeWorkflow(
  host: DeclarativeWorkflowHost,
  workflow: DeclarativeWorkflow,
  options: WorkflowRunOptions = {},
): Promise<unknown> {
  if (workflow.schema_version !== 1 || !workflow.start || !Array.isArray(workflow.nodes)) {
    throw new Error(`Unsupported ${host.kindLabel} workflow schema`);
  }
  ...
  while (current) {
    ...
    if (++steps > MAX_STEPS || Date.now() > deadline) {
      throw new Error(`${host.kindLabel} workflow exceeded its execution limit`);
    }
    const node = nodes.get(current);
    if (!node) {
      throw new Error(`${host.kindLabel} workflow node ${current} does not exist`);
    }
    const next = typeof node.next === 'string' ? node.next : undefined;

    switch (node.type) {
      case 'host_call': {
        const capability = String(node.capability ?? '');
        const args = resolveValue(node.arguments ?? {}, context) as Record<string, unknown>;
        let userConfirmed: boolean | undefined;
        if (host.requiresConfirmation(capability)) {
          if (options.background) {
            throw new Error(`Background workflows cannot call ${capability}`);
          }
          userConfirmed = window.confirm(host.confirmationPrompt(capability, args));
          if (!userConfirmed) throw new Error('Request cancelled by the user');
        }
        const result = await withinExecutionLimit(
          host.executeHostCall(capability, args, userConfirmed),
          deadline,
          options.signal,
        );
      ...
      case 'confirm': {
        if (options.background) {
          throw new Error('Background workflows cannot request confirmation');
        }
      ...
      case 'navigate': {
        if (options.background) {
          throw new Error('Background workflows cannot navigate');
        }
        const target = String(resolveValue(node.to ?? '/home/overview', context));
        if (!target.startsWith('/home/')) {
          throw new Error(`${host.kindLabel} navigation must stay inside the home workspace`);
        }
      ...
      case 'delay': {
        const milliseconds = Math.min(
          Math.max(Number(node.milliseconds ?? 0), 0),
          MAX_DELAY_MS,
        );
      ...
      default:
        throw new Error(`Unsupported ${host.kindLabel} workflow node type ${node.type}`);
    }
  }
  return null;
}
...
async function withinExecutionLimit<T>(
  ...
  if (remaining <= 0) throw new Error('Workflow exceeded its execution limit');
  return new Promise<T>((resolve, reject) => {
    ...
    const cancelled = () => finish(() => reject(new Error('Workflow was cancelled')));
    const timeout = window.setTimeout(
      () => finish(() => reject(new Error('Workflow exceeded its execution limit'))),
      remaining,
    );
...
export function evaluateExpression(
  ...
  const args = Array.isArray(item.args)
    ? item.args.map((value) => evaluateExpression(value, context))
    : [];
  switch (operator) {
    ...
    default: throw new Error(`Unsupported workflow expression operator ${operator}`);
  }
}
